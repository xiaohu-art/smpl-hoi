...
class MotionLib(Command):
    def __init__(
        ...
        self.object: Articulation = env.scene["object"]
        self.env_origin = self.env.scene.env_origins
        self.keypoint_body_index, _ = self.robot.find_bodies(keypoint_body)

        package_dir = os.path.dirname(package_path)
        data = joblib.load(os.path.join(package_dir, "..", motion_clip_dir, data_path))
        print(f"Loaded motion clips from {data_path}")

        bps_path = os.path.join(package_dir, "..", motion_clip_dir, "bps.pt")
    ...
    def sample_init(self, env_ids: torch.Tensor) -> torch.Tensor:
        ...
        init_object_state[:, :3] = self.object_body_pos_w[start_frames].to(self.device) + self.env_origin[env_ids]
        init_object_state[:, 3:7] = self.object_body_quat_w[start_frames].to(self.device)
        init_object_state[:, 7:10] = self.object_body_lin_vel_w[start_frames].to(self.device)
        init_object_state[:, 10:] = self.object_body_ang_vel_w[start_frames].to(self.device)
